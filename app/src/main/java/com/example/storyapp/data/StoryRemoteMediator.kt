...
class StoryRemoteMediator(private val database: StoryDatabase, private val apiService: ApiService, private val token: String): RemoteMediator<Int, ListStoryItem>() {
        ...
        try {
            ...
            database.withTransaction {
                if (loadType == LoadType.REFRESH) {
                    database.remoteKeysStoryDao().deleteRemoteKeys()
                ...
                database.remoteKeysStoryDao().insertAll(keys)
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, ListStoryItem>): RemoteKeys? {
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()?.let { data ->
            database.remoteKeysStoryDao().getRemoteKeysId(data.id)
        }
    }
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, ListStoryItem>): RemoteKeys? {
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.id?.let { id ->
                database.remoteKeysStoryDao().getRemoteKeysId(id)
            }
        }
    }
